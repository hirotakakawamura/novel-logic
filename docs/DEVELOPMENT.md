...
| plot ↔ novel 厳格化 | Phase 0: `novel:<scene>` time 窓のみ Stage 1 厳格、`plot` は hint。Phase 1: `novel_extends_plot`（Tier 1） |
| Tier 1 定理 | `novel_extends_plot` 等（Phase 0 では未実装 — REQUIREMENTS §6.5 / §10.1 #10） |
...
| レビュー | **1 承認必須** + CI green（**PR 作者以外**が Approve） |
| 共同開発者 | Collaborator として追加し、相互レビュー（§13.4） |
...
PR の **1 承認必須**を満たすため、**2 名以上**で相互レビューする。GitHub は **自己承認を拒否**するため、単独では Approve できない。

| 役割 | GitHub 権限 | 責務 |
|------|-------------|------|
| **メンテナー** | Admin（リポジトリオーナー） | ブランチ保護設定、共同開発者の招待、緊急時の admin merge（例外的） |
| **共同開発者** | **Write** 以上 | feature ブランチの push、**他者 PR のレビュー・Approve**、自身 PR の作成 |

**レビューの流れ**

1. 作者が PR を作成 → CI green を待つ
2. **作者以外**の共同開発者（またはメンテナー）がレビューし **Approve**
3. 作者が **Squash merge**（または Approve 者が merge してもよい）

**共同開発者の追加（メンテナー作業）**

GitHub Web:

1. リポジトリ → **Settings** → **Collaborators**（または **Manage access**）
2. **Add people** → GitHub ユーザー名またはメールで招待
3. 権限: **Write**（コード push + PR レビューに十分）
4. 招待を受けた側がメール／通知から **Accept** する

CLI（メンテナー）:

```bash
# USERNAME を招待（Write 権限）
gh api repos/hirotakakawamura/novel-logic/collaborators/USERNAME \
