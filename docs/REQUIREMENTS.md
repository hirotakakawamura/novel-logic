...
> 最終更新: 2026-06-20  
> ステータス: **要件確定（ドメイン）** — 永続化形式・CLI 詳細は実装設計で確定  
...
- Lean の `novel_extends_plot`（Tier 1）は、Phase 0 では scene 1件 ↔ novel 1件を前提に実装する。
...
| time 整合 | action の time が scene / novel の time 区間外 |
...
| plot ↔ novel | scene に紐づく novel 側の fact/action が plot と矛盾しない |
...
| **Lean（Stage 2）** | 時系列上の状態整合、rule の閉包、plot ↔ novel 横断 | 型と定理で整合性を保証 |
...
logic/
  ...
  Project.lean       # 当該作品の ThingId / TimeId / SceneId / PredId（生成）
  Facts.lean         # fixed_fact / state / action の列（生成）
...
証明スタイルは有限列挙の `native_decide` を基本とする（配信の部屋プロトタイプ踏襲）。
...
my-work/
  ...
  actions.yaml
  rules.yaml
  novels.yaml           # novel メタ（scene ごと1件）
  novels/               # 本文（scene ごと1ファイル）
    scene1.txt
...
| `facts.yaml` | fixed_fact / state レコード（`kind`, `thing`, `pred`, `scope`） |
| `actions.yaml` | action レコード（`thing`, `from`, `to`, `at`, `scope`, `label`） |
| `rules.yaml` | rule レコード（`kind`: `forbid-state` / `forbid-transition` 等） |
| `novels.yaml` | novel メタ（`scene_id` キーで **1件のみ**。`time_start`, `time_end`, `body_path`, `revision`, `revisions[]`） |
| `novels/<scene_id>.txt` | 本文（1:1。**git 管理**。論理検証の対象外。CLI は書き込まない） |

**編集ルール**

- CUI 登録コマンドは Stage 1 通過後に上記 YAML を更新する（失敗時は書き込まない）。
- 同一キーへの `add` は拒否し、変更は `update` サブコマンドで行う（fact / action / rule / thing）。
- 本文（`novels/*.txt`）は **YAML に含めない**。エディタで編集し git で管理する。CLI はメタと revision pin のみ。
...
- [ ] Go CLI 骨格
- [ ] ドメインモデル（§3）の YAML スキーマ（§7.3）
- [ ] 登録フロー §4 の CRUD + **登録時矛盾拒否**（Stage 1）
- [ ] Stage 1 検証（rule / time / state 最低限）
- [ ] `novel-logic` 本体に **Core.lean** テンプレ（mathlib なし）
- [ ] Go コード生成器（§6.2: Project / Facts / Rules / Timeline / Theorems）
- [ ] Stage 2: Tier 0 定理 + `lake build`（桃太郎例で end-to-end）
- [ ] `examples/momotaro/` に DRAFT 相当データ
...
| # | 論点 | 備考 |
|---|------|------|
| 8 | **旧モデル再モデル化** | Phase 1 予定（§6.5 Tier 2 / §11）。時期の追加調整のみ |
| 9 | **thing 参照付き fact** | Phase 1（§6.3.1「B が thing 名」） |
